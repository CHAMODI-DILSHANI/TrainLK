...
Router.get("/:scheduleID", async (req, res) => {
  return res.send(await getStationNames(req.params.scheduleID));
});
...
async function getStationNames(scheduleID) {
  const result = await query(
    "select st.stationID, stationName from schedule_has_station sh join station st on sh.stationID = st.stationID where sh.scheduleID = ? order by stationOrder",
    [scheduleID]
  );
  //   console.log(result);
  return result;
